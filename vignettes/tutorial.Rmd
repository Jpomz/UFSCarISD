...
```{r}
num.bins <- nrow(fish_binned)
binBreaks <- c((fish_binned$binMin),
  (fish_binned$binMax)[num.bins])
binCounts <- fish_binned$n


mle_fish_bin <- calcLike(
...
# LBN_bin_plot(binValsTibble = x.binned$binVals, # modify 
...
```{r, eval=FALSE}
dat_object <- NULL # empty object to catch data
...
for(i in 1:length(dat_list)){
  ...
  # now save all the relevant information in  a vector
  # make sure to save the information in the same order as our data frame above: site_name, b, CI_low, CI_high
  dat_out <- data.frame(
    site_name = site_name,
    env_value = env_value,
    b = mle_estimate$MLE, 
    CI_low = mle_estimate$conf[1],
    CI_high = mle_estimate$conf[2])

isd_result_list[[i]] <- dat_out
}
...
```{r}
isd_result_df <- rbind(isd_result_list[[1]],
                       isd_result_list[[2]],
                       isd_result_list[[3]])
isd_result_df
```
