...
3. `fish_binned` is a simulated data set where individual fish were grouped into arbitrary "bins" of body size during the data collection process.  

4. `isd_gradient` is a simulated dataset with body size observations from three "sites", each of which has a different individual size distribution relaitonship. This is used as an example for how to work with data from multiple sites at one time.  

Ypou can load the data objects by running `data(data_name). For example:
